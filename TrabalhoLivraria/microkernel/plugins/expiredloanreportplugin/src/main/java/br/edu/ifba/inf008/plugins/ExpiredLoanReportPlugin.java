...
public class ExpiredLoanReportPlugin implements IPlugin
{
    public boolean init(VBox vbox){
        IUIController uiController = ICore.getInstance().getUIController();
        
        Button reportButton = new Button("Expired Loans Report");
        ...
        reportButton.setOnAction(event -> ExpiredLoanReportScreen());

        return true;
    }

    public void ExpiredLoanReportScreen(){
        IUIController uiController = ICore.getInstance().getUIController();

        Label titleLabel = new Label("Expired Loans");
        titleLabel.setAlignment(Pos.CENTER);
        titleLabel.setStyle("-fx-font-size: 16px; -fx-font-weight: bold;");
        ArrayList<Object> openLoans = ICore.getInstance().getDataController().getExpiredLoans();
